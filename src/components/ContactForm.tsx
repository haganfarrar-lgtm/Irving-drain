...
const ContactForm = () => {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        setStatus('success')
        setFormData({ name: '', phone: '', address: '', budget: '', message: '' })
      } else {
        setStatus('error')
      }
    } catch {
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input type="text" id="name" required value={formData.name}
              ...
              className="w-full px-4 py-3 rounded-xl border border-stone-300 focus:ring-2 focus:ring-stone-500 focus:border-transparent outline-none transition text-stone-800"
          ...
          <div>
            ...
            <input type="text" id="address" required value={formData.address}
          ...
          <div>
            ...
            <select id="budget" required value={formData.budget}
              onChange={(e) => setFormData({ ...formData, budget: e.target.value })}
              className="w-full px-4 py-3 rounded-xl border border-stone-300 focus:ring-2 focus:ring-stone-500 focus:border-transparent outline-none transition text-stone-800 bg-white">
          ...
          <div>
            ...
            <textarea id="message" rows={4} value={formData.message}
              ...
              className="w-full px-4 py-3 rounded-xl border border-stone-300 focus:ring-2 focus:ring-stone-500 focus:border-transparent outline-none transition text-stone-800 resize-none"
              placeholder="Tell us about your project..."></textarea>
          </div>
          <button type="submit" disabled={status === 'sending'}
            className="w-full py-4 bg-stone-800 hover:bg-stone-900 text-white font-bold rounded-xl transition disabled:opacity-50 disabled:cursor-not-allowed text-lg">
            {status === 'sending' ? 'Sending...' : 'Get My Free Estimate'}
          </button>
        </form>
        {status === 'error' && (
          <p className="text-red-500 text-sm mt-3 text-center">Something went wrong. Please call us directly.</p>
        )}
      </div>
    </div>
  )
